    	...
    	function executeScript(scriptString, callbackFn) {
    		...
    		if (scriptString) {
                ...
                for(var i=0;i<lines.length;i++) {
                    if(lines[i].indexOf('AdhocHighchartsAdapter.generateOptions') >= 0) {
                        reportContainer = jQuery('#reportContainer'); 
                        reportContainer.css({position:'absolute',top:0,right:0,bottom:0,left:0});
                        jrPage = jQuery('div.jrPage');
                        if (jrPage.size() != 0) {
                        	var chartUuid = jrPage.find('div.jive_chartSettingsIcon').data('chartuuid');
                        	
                        	jQuery(window).on('resizeEnd',function() {
                        		var chartData = jive.highcharts.chartsData[chartUuid];
                        		if (chartData) {
                        			chartData.highchart.destroy(); // chart should be destroyed first to prevent hicharts errors
                        			chartData.highchart = new Highcharts.Chart(chartData.model);
                        		} else {
                        			// we should not get here
                        			new Highcharts.Chart(window.charts[chartId]);
                        		}
                        	});
                        	
	                        jrPage.css({width:'auto',height:'auto',position:'absolute',top:'32px',right:'4px',bottom:0,left:'4px'});
	                        pageChildren = jrPage.children();
	
	                        if(pageChildren.length == 2) {
	                            var w = reportContainer.width() - 24;
	                            pageChildren.eq(0).css({position:'absolute', top:'-22px', width: w + 'px', height:'24px'});
	                            pageChildren.children().eq(0).css({position:'relative', width:'100%'});
	                            pageChildren.eq(1).css({width:'auto',height:'auto', position:'absolute', top:0, right:0, bottom:0, left:0});
	                        } else {
	                            pageChildren.eq(0).css({width:'auto', height:'auto', position:'absolute', top:0, right:0, bottom:0, left:0});
	                        }
	                        jrPage.find('div.highcharts_parent_container').eq(0).css({position:'absolute',top:0,right:0,bottom:0,left:0});
                        } else {
                        	jrPage = jQuery('table.jrPage');
                        	
                        	var hcParentContainer = jrPage.find('div.highcharts_parent_container'),
                        		chartUuid = jrPage.find('div.jive_chartSettingsIcon').data('chartuuid'),
                        	...
                        	jQuery(window).on('resize', function() {
                        		var hcTimeOut = jive.highcharts.resizeTimeOut; 
                        		hcTimeOut && clearTimeout(hcTimeOut);
                        		hcTimeOut = setTimeout(function() {
                                    jQuery(window).trigger('customResizeEnd');
                                }, 500);
                            });
                        	
                        	jQuery(window).on('customResizeEnd', function() {
                        		var chartData = jive.highcharts.chartsData[chartUuid];
                        		hcParentContainer.css('height', reportContainer.height()-hcParentContainer.position().top);
                        		if (chartData) {
                        			chartData.highchart.destroy(); // chart should be destroyed first to prevent hicharts errors
                        			chartData.highchart = new Highcharts.Chart(chartData.model);
                        		} else {
                        	...
                        	// the first row sets the size for each td => transform td width to percent from table width to properly expand
                        	jrPage.find('tr:first td').each(function(i, v) {
                        		var currentTd = jQuery(v);
                        		currentTd.css('width', (currentTd.width()/pageWidth)*100 + '%');
                        	});
                        	
                        	hcTr.css('height', '100%');	// the row with the chart must be the highest
                        	allRows.each(function(i, v) {	// each row below the one with the chart must have 0px in height
                        		if (i > hcTrIndex) {
                        			jQuery(v).css('height', '0px');
                        		}
                        	});
                        	
                        	jrPage.css({width: '100%', height: '100%'});
                        	hcParentContainer.css('height', (reportContainer.height()-hcParentContainer.position().top) + 'px');
                        }
                        
                        chartId = lines[i].substring(3,lines[i].indexOf('='));
                        lines.splice(i+1,0,'window.charts["'+chartId+'"] = '+chartId+';');
                    }

                    if (lines[i].indexOf('ahIsViewer_highchartsConfig') >= 0) {
                        lines[i] = lines[i].replace("false", "true");
                    }

                    if(chartId && (lines[i].indexOf('chart.width') >= 0 || lines[i].indexOf('chart.height') >= 0)) lines[i] = '//' + lines[i];
                }
                */
